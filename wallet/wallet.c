...
struct wallet *wallet_new(struct lightningd *ld, struct timers *timers)
{
	...
	/* Set early so migration callbacks (e.g. migrate_setup_coinmoves) can use ld->wallet and wallet->db */
	ld->wallet = wallet;
	trace_span_start("db_setup", wallet);
	wallet->db = db_setup(wallet, ld, ld->bip32_base, &wallet->db);
...
bool wallet_extract_owned_outputs(struct wallet *w, const struct wally_tx *wtx,
				  bool is_coinbase,
				  const u32 *blockheight,
				  size_t **outputs)
{
...
void migrate_setup_coinmoves(struct lightningd *ld, struct db *db)
{
	 ...
	 * Read confirmed, unspent wallet-owned UTXOs from the legacy outputs table. */
	...
	db_query_prepared(stmt);
	while (db_step(stmt)) {
		struct utxo *u = tal(utxos, struct utxo);
		db_col_txid(stmt, "prev_out_tx", &u->outpoint.txid);
		u->outpoint.n = db_col_int(stmt, "prev_out_index");
		u->amount = db_col_amount_sat(stmt, "value");
		u32 *bh = tal(u, u32);
		*bh = db_col_int(stmt, "confirmation_height");
		u->blockheight = bh;
