...
/* Override watchman functions - tests don't have real watchman */
static inline u32 test_get_block_height(struct lightningd *ld UNUSED)
{
	return 0;
}
...
static inline void test_watchman_watch_blockdepth(struct lightningd *ld UNUSED,
						  const char *owner UNUSED,
						  u32 confirm_height UNUSED)
{
}
#define watchman_watch_blockdepth test_watchman_watch_blockdepth

static inline void test_watchman_unwatch_blockdepth(struct lightningd *ld UNUSED,
						    const char *owner UNUSED,
						    u32 confirm_height UNUSED)
{
}
#define watchman_unwatch_blockdepth test_watchman_unwatch_blockdepth
...
/* Undefine overrides before auto-generated mocks */
...
/* Generated stub for watchman_unwatch_txid */
void watchman_unwatch_txid(struct lightningd *ld UNNEEDED,
			   const char *owner UNNEEDED,
			   const struct bitcoin_txid *txid UNNEEDED)
{ fprintf(stderr, "watchman_unwatch_txid called!\n"); abort(); }
/* Generated stub for watchman_watch_txid */
void watchman_watch_txid(struct lightningd *ld UNNEEDED,
			 const char *owner UNNEEDED,
			 const struct bitcoin_txid *txid UNNEEDED,
			 u32 start_block UNNEEDED)
{ fprintf(stderr, "watchman_watch_txid called!\n"); abort(); }
...
static bool test_wallet_outputs(struct lightningd *ld, const tal_t *ctx, bool bip86)
{
	...
	CHECK_MSG(test_wallet_add_utxo(w, &u, WALLET_OUTPUT_P2SH_WPKH),
		  "test_wallet_add_utxo failed on first add");
	CHECK_MSG(!wallet_err, wallet_err);

	/* Should fail, we already have that UTXO */
	CHECK_MSG(!test_wallet_add_utxo(w, &u, WALLET_OUTPUT_P2SH_WPKH),
		  "test_wallet_add_utxo succeeded on second add");
	CHECK_MSG(!wallet_err, wallet_err);
	...
	CHECK_MSG(test_wallet_add_utxo(w, &u, WALLET_OUTPUT_OUR_CHANGE),
		  "test_wallet_add_utxo with close_info");
	...
	*blockheight = 100;
	...
	CHECK_MSG(test_wallet_add_utxo(w, &u, WALLET_OUTPUT_P2SH_WPKH),
		  "test_wallet_add_utxo with close_info no commitment_point");
	CHECK_MSG(!wallet_err, wallet_err);
	...
	CHECK_MSG(wallet_add_onchaind_utxo(w, &u.outpoint,
					   ...
					   *u.blockheight,
					   ...
					   NULL,
					   5),
		  "test_wallet_add_utxo with close_info and csv > 1");
	CHECK_MSG(!wallet_err, wallet_err);
	...
	CHECK_MSG(test_wallet_add_utxo(w, &u, WALLET_OUTPUT_P2WPKH),
		  "test_wallet_add_utxo failed, p2wpkh");
