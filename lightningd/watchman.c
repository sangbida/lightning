...
struct pending_op {
	char *op_id;       /* "add:{owner}" or "del:{owner}" */
	char *json_params; /* The JSON params to send to bwatch */
};
					       ...
					       const jsmntok_t *params)
{
	...
	if (!tx)
		return command_fail(cmd, JSONRPC2_INVALID_PARAMS, "Invalid tx hex");
