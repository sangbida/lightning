...
static bool we_broadcast(struct lightningd *ld,
...
static void destroy_tx_broadcast(struct tx_rebroadcast *txrb, struct lightningd *ld)
{
		   ...
		   void *cbarg)
{
	...
	if (get_block_height(ld) + 1 < otx->minblock) {
		log_debug(ld->log, "Deferring broadcast of txid %s until block %u",
			  fmt_bitcoin_txid(tmpctx, &otx->txid),
			  otx->minblock - 1);
