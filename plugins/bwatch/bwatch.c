...
struct rescan_state {
	struct watch *watch;		/* NULL = rescan all watches, non-NULL = single watch */
...
static const jsmntok_t *list_datastore(struct command *cmd, const char **buf_out,
				       const char *key1, const char *key2)
{
	...
	result = jsonrpc_request_sync(tmpctx, cmd, "listdatastore", params, buf_out);
...
static void add_block_to_datastore(struct command *cmd, const struct block_record_wire *br)
{
	...
	char *key = tal_fmt(tmpctx, "bwatch/block_history/%010u", br->height);
	u8 *data = towire_bwatch_block(tmpctx, br);
...
static void load_block_history(struct command *cmd, struct bwatch *bwatch)
{
	...
	datastore = list_datastore(cmd, &buf, "bwatch", "block_history");
...
static char *get_watch_datastore_key(const tal_t *ctx, const struct watch *w)
{
	...
	case WATCH_SCRIPTPUBKEY: {
		...
		return tal_fmt(ctx, "bwatch/%s/%s", type_name, hex);
	}
	case WATCH_OUTPOINT:
		return tal_fmt(ctx, "bwatch/%s/%s", type_name,
			       fmt_bitcoin_outpoint(tmpctx, &w->key.outpoint));
	case WATCH_TXID:
		return tal_fmt(ctx, "bwatch/%s/%s", type_name,
			       fmt_bitcoin_txid(tmpctx, &w->key.txid));
	default:
		abort();
	}
}
				 ...
				 enum watch_type type)
{
	...
	datastore = list_datastore(cmd, &buf, "bwatch", watch_type_name);
...
{
	char *key = get_watch_datastore_key(tmpctx, w);
	struct watch_wire *wire = watch_to_wire(tmpctx, w);
	u8 *data = towire_bwatch_watch(tmpctx, wire);
...
/* Convert slash-separated path to JSON array for datastore key.
 * e.g. "bwatch/txid_watches/abc123" -> ["bwatch", "txid_watches", "abc123"]
 * This matches the format expected by deldatastore/listdatastore RPCs. */
static void json_add_keypath(struct json_stream *js, const char *fieldname,
			     const char *path)
{
	...
	for (size_t i = 0; parts[i]; i++)
		json_add_string(js, NULL, parts[i]);
	json_array_end(js);
}
...
static void delete_watch_from_datastore(struct command *cmd, const struct watch *w)
{
	...
	struct out_req *req = jsonrpc_request_start(cmd, "deldatastore",
						    ...
						    deldatastore_done,
						    NULL);
	json_add_keypath(req->js, "key", key);
	send_outreq(req);

	plugin_log(cmd->plugin, LOG_DBG, "Deleting watch from datastore: %s", key);
}
			     ...
			     int innum)
{
	...
	char *tx_hex = fmt_bitcoin_tx(tmpctx, tx);
			       ...
			       u32 txindex)
{
	...
	if (w && blockheight >= w->start_block)
					   ...
					   u32 *block_height)
{
	...
	if (!block) {
		plugin_log(cmd->plugin, LOG_BROKEN, "Failed to get/parse block %u",
			   *block_height);
						...
						void *unused)
{
	...
	/* Parse the response */
	if (!parse_getchaininfo(buf, result, &blockheight)) {
		plugin_log(cmd->plugin, LOG_DBG, "getchaininfo parse failed, current_height still %u",
			   bwatch->current_height);
		return poll_finished(cmd);
	}
...
/* Remove a watch */
static void remove_watch(struct command *cmd, struct bwatch *bwatch,
			 const struct watch *key, const char *owner_id)
{
	struct watch *w = get_watch(bwatch, key);

	if (!w) {
		plugin_log(cmd->plugin, LOG_UNUSUAL,
			   "Attempted to remove non-existent %s watch",
			   get_watch_type_name(key->type));
		return;
	}
					      ...
					      const jsmntok_t *params)
{
	...
	/* Create template watch for lookup */
	struct watch template;
	template.type = type;
	switch (type) {
	case WATCH_SCRIPTPUBKEY:
		if (!scriptpubkey)
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
					    "scriptpubkey required for type 'scriptpubkey'");
		template.key.scriptpubkey.script = scriptpubkey;
		template.key.scriptpubkey.len = tal_bytelen(scriptpubkey);
		break;
	case WATCH_OUTPOINT:
		if (!outpoint)
	...
	remove_watch(cmd, bwatch, &template, owner);
...
static void json_out_watch_common(struct json_out *jout,
				  ...
				  wirestring **owners)
{
	json_out_addstr(jout, "type", type);
					       ...
					       const jsmntok_t *params)
{
	...
	while (w) {
		...
		json_out_watch_common(jout, "scriptpubkey", w->start_block, w->owners);
		json_out_end(jout, '}');
	...
	while (w) {
		...
		json_out_watch_common(jout, "outpoint", w->start_block, w->owners);
		json_out_end(jout, '}');
	...
	while (w) {
		...
		json_out_watch_common(jout, "txid", w->start_block, w->owners);
		json_out_end(jout, '}');
