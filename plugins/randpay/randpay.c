...
{
	/* 32 bytes = 64 hex chars + null terminator */
	char *payment_hash = tal_arr(tmpctx, char, 65);
	memset(payment_hash, '0', 64);
	payment_hash[64] = '\0';
	return tal_strdup(ctx, payment_hash);
}

/* Helper function to select a random node from the list,
   excluding our own node */
static const char *select_random_node(const char *buf,
	...
	void *udata)
{
	...
	return create_status_response(cmd, ctx, RED,
				      parse_error_message(buf, error));
}
	...
	void *udata)
{
	...
	return create_status_response(cmd, route_data->parent,
				      RED, parse_error_message(buf, error));
}

/* Classifies failcodes into return values */
static enum return_value determine_return_value(struct command *cmd,
						u64 failcode,
						u64 erring_index,
						u64 route_size)
{
	...
	case WIRE_FINAL_INCORRECT_HTLC_AMOUNT:
		plugin_log(cmd->plugin, LOG_INFORM,
			   "Returning RED (final node error)");
		return RED;
	...
	case WIRE_UNKNOWN_NEXT_PEER:
		plugin_log(cmd->plugin, LOG_INFORM,
			   "Returning RED (node unreachable)");
		return RED;
	...
	case WIRE_PERMANENT_CHANNEL_FAILURE:
		plugin_log(cmd->plugin, LOG_INFORM,
			   "Returning YELLOW (liquidity/routing issue)");
		return YELLOW;
	...
	void *udata)
{
	...
	plugin_log(cmd->plugin, LOG_INFORM,
		   "on_waitsendpay_handle called with input: %.*s",
	...
	const jsmntok_t *search_tok;

	/* If no data at top level, look for error.data structure */
	if (!data_tok) {
		const jsmntok_t *error_tok = json_get_member(buf,
							     input,
							     "error");
		if (error_tok)
		{
	...
	failcode_tok = json_get_member(buf, search_tok, "failcode");
	erring_index_tok = json_get_member(buf, search_tok, "erring_index");
	failcodename_tok = json_get_member(buf, search_tok, "failcodename");
	...
	ret = determine_return_value(cmd,
				     failcode,
				     erring_index,
				     ctx->route_size);
	...
	case ERROR:
		return create_status_response(cmd,
					      ctx->parent, ret,
					      failcodename ? tal_fmt(tmpctx,
					      "Error: %s",
					      failcodename) : "Unknown error");
	}
	/* This should never happen, but we need to return something */
	return create_status_response(cmd, ctx->parent, ERROR, "Unknown error");
}
	...
	void *udata)
{
	...
	plugin_log(cmd->plugin,
		   LOG_INFORM,
		   "Sendpay request sent, starting waitsendpay");
	...
	void *udata)
{
	...
	for (int hop_index = 0; hop_index < route_array->size; hop_index++) {
		const jsmntok_t *current_hop = json_get_arr(route_array,
							    hop_index);
		json_object_start(sendpay_req->js, NULL);
		/* Copy hop fields directly */
		json_add_tok(sendpay_req->js,
			     "id",
			     json_get_member(buf, current_hop, "id"),
			     buf);
		json_add_tok(sendpay_req->js,
			     "channel",
			     json_get_member(buf, current_hop, "channel"),
			     buf);
		json_add_tok(sendpay_req->js,
			     "direction",
			     json_get_member(buf, current_hop, "direction"),
			     buf);
		json_add_tok(sendpay_req->js,
			     "amount_msat",
			     json_get_member(buf, current_hop, "amount_msat"),
			     buf);
		json_add_tok(sendpay_req->js,
			     "delay",
			     json_get_member(buf, current_hop, "delay"),
			     buf);
		json_add_tok(sendpay_req->js,
			     "style",
			     json_get_member(buf, current_hop, "style"),
			     buf);
		json_object_end(sendpay_req->js);
	}
	json_array_end(sendpay_req->js);
	/* Add payment hash to request */
	json_add_string(sendpay_req->js,
			"payment_hash",
	...
	void *udata)
{
	...
	/* Check if nodes array is valid, return ERROR if not */
	if (!nodes || nodes->type != JSMN_ARRAY || nodes->size == 0) {
		plugin_log(cmd->plugin,
			   LOG_INFORM,
			   "No nodes found in listnodes response");
		return create_status_response(cmd,
					      ctx,
					      ERROR,
	...
	if (!ctx->node_id) {
		plugin_log(cmd->plugin,
			   LOG_INFORM,
			   "No valid random node selected");
		return create_status_response(cmd,
					      ctx,
					      ERROR,
					      "No valid random node found");
	}
	...
	plugin_log(cmd->plugin,
		   LOG_INFORM,
		   "Requesting route to node %s with amount %" PRIu64 " msat",
		   ctx->node_id,
		   ctx->amount_msat);
	return send_outreq(req);
}
				    ...
				    const jsmntok_t *params)
{
	...
	if (amount_msat && *amount_msat > 0) {
		...
		plugin_log(cmd->plugin,
			   LOG_INFORM,
			   "Starting randpay with amount %" PRIu64 " msat",
			   ctx->amount_msat);
	...
	plugin_log(cmd->plugin,
		   LOG_INFORM,
		   "No amount provided, returning error");
	randpay_ctx *ctx = tal(cmd, randpay_ctx);
	return create_status_response(cmd,
				      ctx,
				      ERROR,
				      "A positive amount_msat parameter is required");
}