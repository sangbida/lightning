...
def test_onchain_missing_no_p2tr_migrate(node_factory, bitcoind):
    ...
    l1 = node_factory.get_node(dbfile='l1-missed-p2wpkh-CLOSINGD_COMPLETE.sqlite3.xz', options={'database-upgrade': True}, old_hsmsecret=True)
    l2 = node_factory.get_node(dbfile='l2-missed-p2wpkh-CLOSED.sqlite3.xz', options={'database-upgrade': True}, old_hsmsecret=True, broken_log="Potentially missing 2 outputs from previous closes: scanning from block 103|Rescan found [0-9a-f:]*!|Rescan finished! 1 outputs recovered.  Let's never do that again")
...
def test_rescan_missing_utxo(node_factory, bitcoind):
    ...
    bitcoind.restore_blocks(blocks)
    l1, _ = node_factory.get_nodes(2, opts={'old_hsmsecret': True})
    l3 = node_factory.get_node(dbfile='l3-missing-utxo.sqlite3.xz', options={'database-upgrade': True}, old_hsmsecret=True)
