...
def test_funding_change(node_factory, bitcoind):
    ...
    bitcoind.generate_block(1, wait_for_mempool=1)
    sync_blockheight(bitcoind, [l1])
    unspent = only_one(l1.db_query('SELECT SUM(satoshis) AS value FROM our_outputs WHERE spendheight IS NULL AND (reserved_til IS NULL OR reserved_til = 0);'))['value']
    spent = only_one(l1.db_query('SELECT SUM(satoshis) AS value FROM our_outputs WHERE spendheight IS NOT NULL;'))['value']

    # The 10m out is spent and we have a change output of 9m-fee
    assert unspent > 8990000
    assert spent == 10000000
